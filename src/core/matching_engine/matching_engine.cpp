...
MatchingEngine::MatchingEngine(Exchange::ClientResponseQueue *rxRequests, Exchange::ClientResponseQueue *txResponses,
                               Exchange::MarketUpdateQueue *txMarketUpdates) noexcept : rxRequests_(rxRequests),
...
void MatchingEngine::dispatchClientResponse(const Exchange::OGSClientResponse *response) noexcept {
...
void MatchingEngine::runMatchingEngine() noexcept {
        ...
        while (isRunning_.load(std::memory_order_relaxed)) {
            const auto request = rxRequests_->pop();
            if (request) [[likely]] {
